...
const brandCloudElements = [...brandLogos, ...brandLogos, ...brandLogos].map((brand, index) => (<div key={`${brand.name}-${index}`} className="flex h-16 w-16 items-center justify-center rounded-2xl border border-white/20 bg-white/95 p-2.5 shadow-lg backdrop-blur-md transition-transform duration-300 hover:scale-110" title={brand.name}>
      <img src={brand.url} alt={brand.name} className="h-full w-full object-contain pointer-events-none" loading="lazy"/>
    </div>));
export const Landing = () => {
    ...
    return (<div className="relative min-h-[220vh] w-full bg-background text-foreground overflow-x-hidden">
      {/* ================= FIXED FLOATING LOGO (V + ianet) ================= */}
      <motion.div style={{ top, left, x, y, scale }} className="fixed z-50 flex origin-top-left items-center tracking-tight pointer-events-auto">
        <img src={logo} alt="V" className="h-10 w-auto object-contain md:h-20"/>
      ...
      <section className="relative flex h-screen w-full flex-col items-center justify-center overflow-hidden border-b">
        ...
        <motion.div style={{ opacity: heroBadgeOpacity }} className="absolute top-[32%] md:top-[30%] z-20 flex items-center justify-center px-4">
        ...
        {/* Globe Background */}
        <motion.div style={{ opacity: globeOpacity }} className="pointer-events-none absolute inset-0 flex items-center justify-center">
