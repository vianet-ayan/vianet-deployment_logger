...
export const getInventoryByAccessGroup = async (accessGroupId: number, limit = 500, offset = 0) => {
  const result = await query(
    `SELECT
       iag.id           AS iag_id,
       iag.accessgroupid,
     ...
     ORDER BY iag.id ASC
     LIMIT $2 OFFSET $3`,
    [accessGroupId, limit, offset]
  )
  return result.rows
}

export const getInventoryByAccessGroupCount = async (accessGroupId: number): Promise<number> => {
  const result = await query(
    `SELECT COUNT(*) FROM app.inventory_access_group WHERE accessgroupid = $1`,
    [accessGroupId]
  )
