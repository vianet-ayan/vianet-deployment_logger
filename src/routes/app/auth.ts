...
import { getAppAccount } from '../../db/main.js';
...
router.get('/', (_req, res) => {
  res.json({ route: 'app-auth', status: 'working' });
});
...
router.post('/email', async (req, res) => {
  ...
  const result = await getAppAccount(email)
  if (!result) {
...
router.post('/jwt', async (req, res) => {
  ...
  const account = await getAppAccount(email)
  if (!account) {
    res.status(404).json({ error: 'Account not found' })
    return
  }
...
router.post('/validate-token', async (req, res) => {
  ...
  try {
    const decoded = verifyToken(token)
    const account = await getAppAccount(decoded.email)
