    ...
    <div className="border rounded-lg p-3 mb-2 cursor-pointer hover:bg-muted/50" onClick={onToggle}>
      ...
      {isOpen && (
        <div className="mt-2 pt-2 border-t text-xs text-muted-foreground">
          ...
          {transaction.inventoryentries.length > 0 && (
            <p>Items: {transaction.inventoryentries.map(ii => ii.stockItemName).join(', ')}</p>
          )}
        </div>
      )}
    </div>
...
function DetailTab({ filtered, expandedIds, toggleExpand }: { filtered: DaybookEntry[]; expandedIds: Set<string>; toggleExpand: (id: string) => void }) {
...
export default function DayBook() {
  ...
  const dailyTotals = useMemo(() => {
    ...
    for (const t of transactionsData) {
      ...
      if (t.voucher_type === 'Sales' || t.voucher_type === 'Sales HP') daily[day].income += getAmount(t)
      else daily[day].expense += getAmount(t)
    }
    return Object.entries(daily).map(([day, v]) => ({ day, income: v.income, expense: v.expense }))
  }, [transactionsData])

  const filtered = useMemo(() =>
    transactionsData.filter((t: DaybookEntry) =>
  ...
  const totalSales = useMemo(() => transactionsData.filter((t: DaybookEntry) => t.voucher_type === 'Sales' || t.voucher_type === 'Sales HP').reduce((s: number, t: DaybookEntry) => s + getAmount(t), 0), [transactionsData])
  const totalPayments = useMemo(() => transactionsData.filter((t: DaybookEntry) => t.voucher_type === 'Payment').reduce((s: number, t: DaybookEntry) => s + getAmount(t), 0), [transactionsData])
  const totalExpenses = useMemo(() => transactionsData.filter((t: DaybookEntry) => t.voucher_type === 'Purchase').reduce((s: number, t: DaybookEntry) => s + getAmount(t), 0), [transactionsData])
